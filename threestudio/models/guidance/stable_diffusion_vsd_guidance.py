...
class StableDiffusionVSDGuidance(BaseModule):
    ...
    def configure(self) -> None:
        ...
        pipe_lora_kwargs = pipe_kwargs.copy()
        ...
        threestudio.info("Initializing PEFT LoRA adapters ...")
        ...
        lora_config = LoraConfig(
            ...
            lora_dropout=0.0,
            bias="none",
            task_type="UNET",  # 自定义类型，可为 "UNET" 或 "CAUSAL_LM"
            target_modules=["to_q", "to_k", "to_v", "to_out.0"],
        )

        # 包装为 PEFT 模型
        unet = self.submodules.pipe_lora.unet
        unet = get_peft_model(unet, lora_config)
        self.submodules.pipe_lora.unet = unet  # 替换
        ...
        threestudio.info("Loaded Stable Diffusion + PEFT LoRA successfully!")
        ...
        with torch.no_grad():
            ...
            with self.disable_unet_class_embedding(self.unet) as unet:
                ...
                noise_pred_pretrain = self.forward_unet(
                    ...
                    cross_attention_kwargs=cross_attention_kwargs,
                    class_labels = torch.zeros((2 * B, camera_condition.view(B, -1).shape[-1]), device=self.device, dtype=camera_condition.dtype)
